...
let handleForm = () => {
    ...
    if (isAllDigits(username.value) && username.value.length != 11) {
...
<template>
    ...
    <form class="form" @submit.prevent="handleForm">
        <InputBox title="شماره موبایل یا نام کاربری" v-model="username">
        ...
        <RouterLink :to="{ name: 'auth.forget-password' }" class="forgetpass_link">رمز عبور خود را فراموش کرده اید؟
        </RouterLink>

        <button class="submitButton">
            {{ !isLoading ? 'ورود به حساب کاربری' : '' }}
